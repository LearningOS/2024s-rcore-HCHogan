...
use crate::{
    ...
    task::{
        add_task, current_task, current_user_token, exit_current_and_run_next, get_start_time,
        get_syscall_times, mmap, munmap, suspend_current_and_run_next, TaskStatus,
    },
    timer::get_time_us,
};
...
pub fn sys_task_info(ti: *mut TaskInfo) -> isize {
    ...
    let task_info = TaskInfo {
        status: current_task().unwrap().inner_exclusive_access().task_status,
        syscall_times: get_syscall_times(),
        time: get_time_us() - get_start_time(),
    };
...
pub fn sys_spawn(path: *const u8) -> isize {
    ...
    let Some(data) = get_app_data_by_name(&translated_str(current_user_token(), path)) else {
        return -1;
    };
    let current_task = current_task().unwrap();
    let new_task = current_task.fork();
...
pub fn sys_set_priority(prio: isize) -> isize {
    ...
    current_task().unwrap().inner_exclusive_access().set_prio(prio as usize);
    prio
}
